...
class Image_V1:
	...
	"""
	def __init__(self, frames_number: int = 3, frame_size: tuple = (270, 480, 3), learning_rate: float = 1e-5):
		"""
	...
	def train(self, frames: np.ndarray, labels: np.ndarray, batch_size: int = 128, epochs: int = 10, batch_retrains: int = 5, verbose: int = 1):
		"""
	...
	def test_model(self,  frames: np.ndarray, labels: np.ndarray, batch_size: int = 1024):
		"""
	...
	def interference(self, video_filename: str, batch_size: int = 1024, verbose: int = 1):
		...
		while cap.isOpened():
			...
			if (len(temp_frames) == batch_size+self.frames_number):
				X = np.lib.stride_tricks\
				  .sliding_window_view(np.array(temp_frames, dtype='i1'), self.mega_frame_size)\
